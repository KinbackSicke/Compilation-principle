...
void display(struct ASTNode *T, int indent)
{
	...
	if (T)
	{
		...
		case EXT_VAR_DEF:   
			printf("%*c外部变量定义：(%d)\n",indent,' ',T->pos);
            display(T->ptr[0], indent + 2);        //显示外部变量类型
            printf("%*c变量名：\n",indent + 2,' ');
            display(T->ptr[1], indent + 4);        //显示变量列表
            break;
		...
		case FUNC_DEF:      
			printf("%*c函数定义：(%d)\n",indent,' ',T->pos);
            display(T->ptr[0], indent + 2);      //显示函数返回类型
            display(T->ptr[1], indent + 2);      //显示函数名和参数
            display(T->ptr[2], indent + 2);      //显示函数体
            break;
		...
		case FUNC_DEC:      
			...
			{
                printf("%*c函数形参：\n", indent,' ');
                display(T->ptr[0], indent + 2);  //显示函数参数列表
            }
            else 
				printf("%*c无参函数\n", indent + 2, ' ');
            break;
		...
		case COMP_STM:      
			printf("%*c复合语句：(%d)\n", indent, ' ', T->pos);
			printf("%*c复合语句的变量定义部分：\n", indent + 2, ' ');
			display(T->ptr[0], indent + 4);      //显示定义部分
			printf("%*c复合语句的语句部分：\n", indent + 2, ' ');
			display(T->ptr[1], indent + 4);      //显示语句部分
			break;
		...
		case WHILE:         
			printf("%*c循环语句：(%d)\n", indent, ' ', T->pos);
            printf("%*c循环条件：\n",indent + 2,' ');
            display(T->ptr[0], indent + 4);      //显示循环条件
            printf("%*c循环体：(%d)\n",indent + 2,' ', T->pos);
            display(T->ptr[1], indent + 4);      //显示循环体
            break;
			
		case IF_THEN:       
			printf("%*c条件语句(IF_THEN)：(%d)\n", indent, ' ', T->pos);
            printf("%*c条件：\n", indent + 2,' ');
            display(T->ptr[0], indent + 4);      //显示条件
            printf("%*cIF子句：(%d)\n", indent + 2, ' ', T->pos);
            display(T->ptr[1], indent + 4);      //显示if子句
            break;
		...
		case IF_THEN_ELSE:  
            ...
            display(T->ptr[1], indent + 4);      //显示if子句
            printf("%*cELSE子句：(%d)\n", indent + 2, ' ', T->pos);
            display(T->ptr[2], indent + 4);      //显示else子句
            break;
		
		case CONTINUE:
			printf("%*c\"continue\"语句(%d)\n", indent, ' ', T->pos);
			break;
		...
		case VAR_DEF:      
			printf("%*c局部变量定义：(%d)\n", indent, ' ', T->pos);
            display(T->ptr[0], indent + 2);   //显示变量类型
            display(T->ptr[1], indent + 2);   //显示该定义的全部变量名
            break;
...
//          printf("%*c第%d个实际参数表达式：\n",indent,' ',i);
  //        display(T,indent+2);
		...
		case ARRAY_DEC:
			printf("%*c数组定义(%d)：\n", indent, ' ', T->pos);
			printf("%*c数组长度(%d)：%d\n", indent, ' ', T->pos, T->array_size);	
			break;
		
		case ARRAY_VIS:
			printf("%*c数组访问(%d)：\n", indent, ' ', T->pos);
			printf("%*c数组名(%d)：%s\n", indent, ' ', T->pos, T->type_id);
			display(T->ptr[0], indent + 2);
			break;
